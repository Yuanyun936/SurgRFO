...
# Make sure we can import train.py from the same folder
import sys
...
# in all test-time analyses in this script (treated as "no detection").
MIN_BOX_W = 1.0
MIN_BOX_H = 1.0
...
def run_inference(model, loader, device) -> Tuple[List[Dict], List[Dict]]:
    ...
    for images, _targets in loader:
        ...
        for out in outputs:
            ...
            # "no detection" in all downstream metrics and visualizations.
            if pb.size > 0:
                ws = pb[:, 2] - pb[:, 0]
                hs = pb[:, 3] - pb[:, 1]
...
def roc_auc_binary(y_true: np.ndarray, y_score: np.ndarray) -> Optional[float]:
    ...
    Uses a standard rank-based trapezoidal integration over the ROC curve.
    Returns None if there are no positive or no negative samples.
    """
...
def gt_boxes_from_dataset_items(ds) -> List[Dict[str, np.ndarray]]:
    """Extract ground-truth boxes/labels per image from LabelMe JSON.

    Only the 'sponge' class is used (label id = 1).
    Returns list of {"boxes": np.ndarray[N,4], "labels": np.ndarray[N]}.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Test RetinaNet on RFO sponge dataset using LabelMe JSON "
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="./retina_sponge_test_out",
    ...
    # Metrics thresholds (must match or be comparable to training time)
    parser.add_argument("--score-thr", type=float, default=0.5)
    parser.add_argument("--iou-thr", type=float, default=0.5)

    # Manifest selection & strict mode (same semantics as train.py)
    parser.add_argument(
        "--manifest-file",
        type=str,
        default=None,
    ...
    # Image-level GT and predictions
    ...
    with open(out_dir / "README.txt", "w", encoding="utf-8") as f:
        f.write(
            "Files generated by retina/test.py\n"
