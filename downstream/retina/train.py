...
# Minimum box size (in pixels) to consider a detection valid.
# Boxes with width < MIN_BOX_W or height < MIN_BOX_H will be ignored
# in all evaluate()-time analyses (treated as "no detection").
MIN_BOX_W = 1.0
MIN_BOX_H = 1.0
...
def iou_xyxy(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    area_b = (bx2 - bx1) * (by2 - by2 + (by2 * 0))  # placeholder to avoid lint; replaced below
...
def ap_iou50_single_class(preds, gts, class_id: int, score_thr: float = 0.5, iou_thr: float = 0.5):
    ...
    preds, gts: lists of dicts from detector output / targets.
    """
    ...
    def __init__(
        ...
        if rows:
            for r in rows:
                ...
                if img_path is None:
                    # Read LabelMe JSON to get imagePath field
                ...
                if img_path is None:
                    # Fallback: use stem with common extensions
                    for ext in IMG_EXTS:
...
def create_model(num_classes: int = 2, pretrained: bool = True) -> torch.nn.Module:
    """Create RetinaNet with a ResNet50-FPN backbone.

    注意：在较老版本的 torchvision 中，`pretrained=True` 会加载 COCO 上
    训练好的完整模型（包含 91 类分类头），这与我们自定义的 `num_classes`
    不匹配，会在 `load_state_dict` 时触发 size mismatch 错误。

    这里的策略是：
    - 仍然使用预训练的 backbone（`pretrained_backbone=True`）；
    - 但不加载预训练的检测头（`pretrained=False`），让分类/回归头随机初始化，
      并使用我们指定的 `num_classes` 重新构建头部。
    """

    if pretrained:
        # 仅加载预训练 backbone，检测头使用随机初始化且匹配 num_classes
        try:
            ...
            # 兼容非常新的/非常旧的 torchvision 签名（没有 pretrained_backbone 参数）
            model = retinanet_resnet50_fpn(
                pretrained=False,
                num_classes=num_classes,
            )
...
def evaluate(model, loader, device, score_thr: float = 0.5, iou_thr: float = 0.5) -> Dict[str, float]:
    ...
    for images, targets in loader:
        ...
        for o, t in zip(outs, targets):
            ...
            # Filter out degenerate / tiny boxes so they are treated as
            # "no detection" in all downstream metrics.
            if boxes.numel() > 0:
                ws = boxes[:, 2] - boxes[:, 0]
                hs = boxes[:, 3] - boxes[:, 1]
...
def _get_loss_component(loss_dict, keys: List[str]) -> torch.Tensor:
    for k in keys:
        if k in loss_dict:
            return loss_dict[k]
    return torch.zeros((), dtype=torch.float32, device=list(loss_dict.values())[0].device)
...
def train_one_epoch(model, loader, optimizer, device, scheduler=None):
    ...
    for step, (images, targets) in enumerate(loader):
        images = [img.to(device) for img in images]
        # Detector ignores image-level label; do not pass it to the model
        ...
        # RetinaNet: keys are usually "classification" and "bbox_regression"
        cls_loss_t = _get_loss_component(loss_dict, ["classification", "loss_classifier"])
        box_loss_t = _get_loss_component(loss_dict, ["bbox_regression", "loss_box_reg"])
        rpn_cls_t = _get_loss_component(loss_dict, ["loss_objectness"])  # 可能不存在
        rpn_box_t = _get_loss_component(loss_dict, ["loss_rpn_box_reg"])  # 可能不存在

        loss_sums["loss"] += float(losses.detach().cpu())
        loss_sums["cls"] += float(cls_loss_t.detach().cpu())
        loss_sums["box_reg"] += float(box_loss_t.detach().cpu())
        loss_sums["rpn_cls"] += float(rpn_cls_t.detach().cpu())
        loss_sums["rpn_bbox"] += float(rpn_box_t.detach().cpu())
        n += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "RetinaNet on RFO sponge dataset using LabelMe JSON. "
